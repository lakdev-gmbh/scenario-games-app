...
export default class TaskDB extends Model {
  ...
  @text('type') type;
  @json('options', castOptions) options;
  ...
  @writer async addUserAnswer(answer, isCorrect) {
    const existingUserAnswers = await this.collections
      .get('user_completed_tasks')
      .query(Q.where('task_watermelon_id', this.id))
      .destroyAllPermanently();
    const newUserAnswer = await this.collections
      .get('user_completed_tasks')
      .create(userCompletedTask => {
