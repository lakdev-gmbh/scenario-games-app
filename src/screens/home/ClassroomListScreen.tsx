    ...
    <TouchableOpacity
      ...
      <View style={styles.classroomSubjects}>
        {subjects.map(subject => (
          <FatTag key={subject.id}>{subject.name}</FatTag>
        ))}
        {levels.map(level => (
          <FatTag secondary key={level?.id}>
            {level?.name}
          </FatTag>
        ))}
      </View>
...
export const ClassroomListScreen = () => {
  ...
  const subjectsInClass = (classroom: UserGroup) => {
    const subjects = classroom.scenarios
      .map((scenario: Scenario) => scenario.subjects)
      .flat();
  ...
  const levelsInClass = (classroom: UserGroup) => {
    const levels = classroom.scenarios
      .map((scenario: Scenario) => scenario.classLevel)
      .flat();
    const levelsDistinct = levels.filter(distinctObjects);
    return levelsDistinct;
  };

  const distinctObjects = (v: any, i: number, self: any[]): boolean =>
    v && self.findIndex(t => t.name === v.name) === i;
  ...
  const getScenarios = async () => {
    const newClassrooms = await watermelondb
      .get('app_classrooms')
      .query()
      .fetch();
    const newUserGroups = await Promise.all(
      newClassrooms.map(async app_classroom => {
  ...
  const addClassroom = async () => {
    const user_groups = await watermelondb
      .get('user_groups')
      .query(Q.where('code', accessCode))
      .fetch();
    ...
    if (user_groups.length === 0) {
      ...
      await watermelondb.write(async () => {
        const localData = await watermelondb
          .get('app_classrooms')
          .query(Q.where('user_group_watermelon_id', user_group.id))
          .fetchCount();
        if (localData > 0) {
  ...
  const oneClassroomFragment = activeClassroom && (
    <Fragment>
      <View style={[globalStyles.borderTop, styles.classroomContainer]}>
        <ClassroomItem
  ...
  return (
    <ListScreen
      scenarios={activeClassroom ? activeClassroom.scenarios : scenarioData}
